...
class BaseModel(nn.Module):
    ...
    def freeze(self, transfer_type):
        ...
        if transfer_type == "no_freeze":
            ...
            for name, param in self.clip_model.named_parameters():
                if "adapter" in name or "lora" in name:      #  embeds
                    param.requires_grad = True
                else:
                    param.requires_grad = False

        elif "partial" in transfer_type:
            total_layer = len(self.clip_model.visual.transformer.resblocks) # total number of resblocks [ViT/B-16 = 12]
            partial_layer = int(transfer_type.split("-")[-1]) # last few number of resblocks to be trained
            if partial_layer > total_layer:
                raise NotImplementedError
            for name, param in self.clip_model.named_parameters():
                find = False
                for l in range(total_layer-partial_layer, total_layer): # range(12-X, 11)
